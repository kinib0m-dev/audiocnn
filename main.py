...
model_volume = modal.Volume.from_name("esc-model")  # For model checkpoints
...
class AudioClassifier:
    ...
    def inference(self, request: InferenceRequest):
        # Production: Frontend -> upload file to s3 -> inference endpoint -> download from s3
        ...
        with torch.no_grad():
            ...
            probabilities = torch.softmax(output, dim=1) # Dim=0 batch, dim=1 class
            ...
            if len(audio_data) > max_sample:
                step = len(audio_data) // max_sample
...
def main():
    audio_data, sample_rate = sf.read("chickens.wav")
